...
$setup = ("".$path."/".$div."/".$folder."/details/setup.txt");
...
<?php
  ...
  echo "<br><span style=\"position:relative;align: right;\">Price: $".$unitprice."<span>";
  if (file_exists($setup)){
  echo "<br><span style=\"font-size:9px\">*Plus one time set-up charge of $".file_get_contents($setup)."</spam>";
  }
...
<?php
  ...
  if (file_exists($york)) {
   ...
   $replacementcost = file_get_contents($replace);
   }
   else {
   $replacementcost = 0;
   }
   ...
   // new york option
   $yorked = "<br><span style=\"position:relative;float:left\"><input id=\"".trim($myText)."xxYork\" type=\"checkbox\" onclick=\"yorker( '".trim($replacementcost)."' , '".trim($myText)."');\">New York Option</span><br>";
...
if (file_exists($replace)) {
   $replacementcost = file_get_contents($replace);
   $theCost = str_replace("dot",".",$replacementcost);  
   // new york option
   $yorked .= "<br><span style=\"position:relative;float:left\"><input id=\"".trim($myText)."xxReplace\" type=\"checkbox\" onclick=\"yorker('".trim($replacementcost)."' , '".trim($myText)."');\">Replacement Graphic Only - Price: $".trim($theCost)."</span><br>";
  ...
  {
  echo ("<img style=\"cursor:pointer;\" name=\"add_button\" id=\"".trim($linker)."\" src=\"./assets/images/cart.png\" onclick=\"addtocart(this.id);\">");
  echo $yorked;
